...
print("Best cross-validation score: {:.4f}".format(svm_grid.best_score_))
...
y_pred_proba = best_svm.predict_proba(X_test_final)[:, 1]
...
experiment_results = {
    'Model': 'SVM + LSTM Autoencoder + Feature Fusion',
...
with open('hasil_table_untuk_jurnal.txt', 'w') as f:
    ...
    f.write(
        "*Note: The proposed model combines SVM with LSTM Autoencoder feature extraction and custom feature fusion techniques.")

print("\nProses selesai! Model terbaik tersimpan sebagai 'best_svm_model.joblib'")
...
def predict_diabetes(new_data):
    ...
    # Prediksi
    prediction = best_svm.predict(X_new_final)
    probability = best_svm.predict_proba(X_new_final)[:, 1]
